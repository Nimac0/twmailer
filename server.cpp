...
int main(int argc, char **argv)
{
    ...
    while (!abortRequested)
    {
        ...
        // Makes new thread, new int -> no race conditions bc of overwriting
        if ((pthread_create(&newThread, NULL, clientCommunication, static_cast<void *>(new ThreadArgs{new int(new_socket), std::string(inet_ntoa(cliaddress.sin_addr)), std::string(argv[2])}))) != 0)
        {
            perror("thread error");
        }
